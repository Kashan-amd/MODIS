...
new class extends Component {
    ...
    public function saveJournalEntry()
    {
        ...
        if (abs($totalDebit - $totalCredit) > 0.001) {
            $this->addError('entries', 'Journal entries must balance (total debits must equal total credits)');
            return;
        }
        ...
        try {
            ...
            DB::rollBack();
            $this->addError('general', 'Error creating journal entry: ' . $e->getMessage());
        }
    }

    public function viewJournalDetails($transactionId)
    {
        ...
        $this->modal('journal-details')->show();
    }
    ...
    public function generatePdf($transactionId)
    {
        ...
        $pdf = PDF::loadView('livewire.accounting.pdf.journal-entry', [
            'transaction' => $transaction,
        ]);

        $filename = 'journal-entry-' . $transaction->id . '.pdf';

        return response()->streamDownload(fn() => print $pdf->output(), $filename);
    }

    public function reverseJournalEntry($transactionId)
    {
        DB::beginTransaction();
        try {
            ...
            DB::commit();
            $this->dispatch('journal-entry-reversed', 'Journal entry reversed successfully');

            // Close the modal if it's open
            if ($this->viewingTransactionId === $transactionId) {
                $this->modal('journal-details')->close();
                $this->viewingTransactionId = null;
                $this->transactionDetails = null;
            }
        } catch (\Exception $e) {
            DB::rollBack();
            $this->addError('general', 'Error reversing journal entry: ' . $e->getMessage());
        }
    }
    ...
    public function createFromJobBooking($jobId)
    {
        ...
        $this->reference = 'JOB-' . $job->job_number;
        $this->description = 'Journal entry for job: ' . $job->campaign;

        $this->modal('journal-form')->open();
    }
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            ...
            <div class="mb-4 sm:mb-0">
                <div class="flex items-center space-x-2">
                    ...
                    <h1 class="text-2xl md:text-3xl font-extrabold text-slate-800 dark:text-slate-100">Journal Entries
                    </h1>
                </div>
            ...
            <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
                ...
                <flux:modal.trigger name="journal-form">
        ...
        <x-glass-card colorScheme="indigo" class="backdrop-blur-sm mb-8">
            <div class="flex flex-col md:flex-row md:items-center justify-between mb-6">
                <h2 class="text-lg font-semibold text-indigo-200 mb-3 md:mb-0">Journal Entries</h2>
                <span class="text-sm text-indigo-300">{{ $journals->total() }}
                    {{ Str::plural('entry', $journals->total()) }}</span>
            </div>
        ...
        <flux:modal name="journal-form" class="w-full max-w-4xl">
            <x-glass-card colorScheme="indigo" class="overflow-hidden">
                <form wire:submit="saveJournalEntry">
                    ...
                    <div class="flex justify-between items-center border-b border-indigo-200/20 p-4">
                        <h3 class="text-lg font-semibold text-indigo-100">Create Journal Entry</h3>
                    </div>
                    ...
                    <div class="p-5 space-y-6">
                        ...
                        <div class="mt-6">
                            <div class="flex justify-between items-center mb-3">
                                <h3 class="text-lg font-semibold text-indigo-200">Journal Entries</h3>
