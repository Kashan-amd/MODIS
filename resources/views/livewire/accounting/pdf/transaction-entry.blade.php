        ...
        <tbody>
            @foreach ($transaction->entries as $entry)
                <tr>
                    ...
                    <td class="amount">{{ $entry->debit > 0 ? $entry->debit : '' }}</td>
                    <td class="amount">{{ $entry->credit > 0 ? $entry->credit : '' }}</td>
                </tr>
            @endforeach
            <tr class="total-row">
                <td colspan="2" style="text-align:right">Totals</td>
                <td class="amount">{{ $transaction->entries->sum('debit') }}</td>
                <td class="amount">{{ $transaction->entries->sum('credit') }}</td>
            </tr>
        </tbody>
    </table>

    <div class="footer">
        <p>Generated by MODIS Accounting System</p>
