...
new class extends Component {
    ...
    {
        $this->reset([
            'job_number', 'campaign', 'client_id', 'sale_by', 'po_number',
            'approved_budget', 'gst', 'status', 'editingJobId', 'isEditing'
        ]);
        $this->resetItemsForm();
    ...
    {
        $query = JobBooking::with(['client', 'jobCostings'])
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                                        <div class="md:col-span-1">
                                            ...
                                            <flux:select wire:model="items.{{ $index }}.vendor_id"
                                                ...
                                                <flux:select.option value="{{ $vendor->id }}">{{ $vendor->name }}
                                                </flux:select.option>
                                                @endforeach
                                            </flux:select>
                                        ...
                                        <div class="md:col-span-1">
                                            ...
                                            <flux:select wire:model="items.{{ $index }}.item_id"
                                                ...
                                                <flux:select.option value="{{ $itemOption->id }}">{{ $itemOption->name
                                                    }}</flux:select.option>
                                                @endforeach
                                            </flux:select>
        ...
        <x-glass-card colorScheme="indigo" class="backdrop-blur-sm">
            <div class="flex flex-col md:flex-row md:items-center justify-between mb-6">
                ...
                <div class="flex items-center">
                    <span class="text-sm text-indigo-300 mr-2">{{ $jobBookings->total() }} {{ Str::plural('job booking',
                        $jobBookings->total()) }}</span>
                </div>
            </div>
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div
                                    ...
                                    <h3 class="text-xl font-medium text-slate-800 dark:text-slate-200">No job bookings
                                        found</h3>
